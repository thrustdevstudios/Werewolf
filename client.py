...
async def get_msg(id):    
    return translations[id][random.choice()]
...
async def closegame(ctx):
    ...
    for player in player_list:
        user = client.get_user(player)
        await user.send(f"The game you were in was closed by the session host. Thanks for playing!\n\n_powered by Luftways_")
...
async def leave(ctx):
    ...
    if game_status == "waiting":
        ...
        player_dict.popitem(ctx.message.author.id)
        log(0, f"Updated player dictionary: {player_dict}")
